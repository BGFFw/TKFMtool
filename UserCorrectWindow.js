...
ui.layout(
    <frame>
        <vertical>
            <appbar>
                <toolbar id="toolbar" title="识图结果纠错" />
            </appbar>
            <list id="todoList">
                <card w="*" h="70" margin="10 5" cardCornerRadius="2dp"
                    ...
                    <horizontal gravity="center_vertical">
                        ...
                        <vertical padding="10 8" h="auto" w="0" layout_weight="1">
                            <text id="title" text="{{this.title}}" textColor="#999999" textSize="16sp" maxLines="1" />
                            <text text="{{this.summary}}" textColor="#222222" textSize="16sp" maxLines="1" />
                        </vertical>
...
var storage = storages.create("chosed");
//从storage获取todo列表
var todoList = storage.get("UserCorrectSet");
...
ui.todoList.on("item_bind", function (itemView, itemHolder) {
    ...
    itemView.done.on("check", function (checked) {
        ...
        item.done = checked;
        storage.put("UserCorrectSet", todoList);
...
ui.add.on("click", () => {
    dialogs.rawInput("请输入想改正的识图结果")
        .then(title => {
            if (!title) {
                return;
            }
            dialogs.rawInput("请输入改正后结果")
                .then(summary => {
                    todoList.push({
                        title: title,
                        summary: summary,
